...
import { GmvMaxCampaignRow, TimeWindow } from "@/lib/types";
import { WINDOW_KEYS, WINDOW_LABELS, formatNumber } from "@/lib/labels";

export default function GmvMaxPage() {
  const [window, setWindow] = useState<TimeWindow>("last_7d");
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/gmvmax?window=${window}`);
  ...
  const summary = useMemo(() => {
    ...
    const orders = filtered.reduce((a, x) => a + x.metrics.orders, 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">LIVE GMV Max</h1>
          <p className="text-sm text-neutral-400">
            แคมเปญ GMV Max โดยเฉพาะ — GMV/ROAS จริง + เปิด-ปิดได้
          </p>
        </div>
      ...
      {loading && (
        <div className="card text-sm text-neutral-400">
          กำลังดึงข้อมูล GMV Max… (ดึงชื่อ+สถานะทีละแคมเปญ อาจใช้เวลาสัก 5-15
          วินาทีถ้ามีหลายแคมเปญ)
        </div>
      )}

      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        <Stat label="GMV (ยอดขายรวม)" value={formatNumber(summary.gmv)} highlight />
        <Stat label="ค่าโฆษณา" value={formatNumber(summary.spend)} />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-800 text-left text-xs uppercase tracking-wide text-neutral-500">
              ...
              <th className="p-3 text-right">ค่าโฆษณา</th>
                ...
                >
                  <td className="p-3">
                    ...
                    <div className="text-xs text-neutral-500">
                      {row.shopping_ads_type || "GMV Max"} · ID:{" "}
                      {row.campaign_id}
                    </div>
                  </td>
                  ...
                  <td className="p-3 text-right">
                    {formatNumber(row.metrics.orders, 0)}
                  </td>
